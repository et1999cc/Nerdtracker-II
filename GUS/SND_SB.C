...
#define SB_DSPCOMM_DMAMODEDACLO_AI 0x1C
#define SB_DSPCOMM_DMAMODEDACHI 0x91
#define SB_DSPCOMM_DMAMODEDACHI_AI 0x90
...
void    SB_CallAfter(int CurrentBuffNum);
...
int     SB_Initialize(void)
        ...
        // Wait 3 microseconds!
        // delay(3);
        inp(SB_BaseAddr+SB_DSP_RESETPORT);
        inp(SB_BaseAddr+SB_DSP_RESETPORT);
        inp(SB_BaseAddr+SB_DSP_RESETPORT);
        inp(SB_BaseAddr+SB_DSP_RESETPORT);
...
int     SB_CallBefore(int CurrentBuffNum)
        ...
        SB_WriteDSP(TimeConst);
        //if(Rate<21739)
        //  SB_WriteDSP(SB_DSPCOMM_DMAMODEDACLO_AI);
        //else
          SB_WriteDSP(SB_DSPCOMM_SETBLKSIZE);
          //SB_WriteDSP(SB_DSPCOMM_DMAMODEDACHI);
        ...
        if(Rate>=21739)
          SB_WriteDSP(SB_DSPCOMM_DMAMODEDACHI_AI);
        else
          SB_WriteDSP(SB_DSPCOMM_DMAMODEDACLO_AI);
...
//        return(MixBuffNum+(DMABuffNum<<1)+(Order<<2));
        return(DMABuffNum+(MixBuffNum<<1)+(Order<<2));
        }
...
int     SB_AutoDetect(SND_DeviceTable *DevTab)
        ...
        (*DevTab).Flags=SND_DEV_UNSIGNEDOUTPUT+SND_DEV_AIDMA;
